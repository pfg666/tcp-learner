...
import learner.exceptions.ConfigurationException;
import learner.exceptions.Messages;
import learner.exceptions.OutOfTimeException;
import learner.exceptions.UpdateConstantActionException;
...
public class Main {
	...
	public static void learn_nodata(Config config) throws FileNotFoundException {
		...
		logger.info("maximum learning time (seconds): "
				+ config.learning_maxTime);
		...
		RandomWalkEquivalenceOracle eqOracle = new RandomWalkEquivalenceOracle(
				config.testing_maxNumTraces, config.testing_minTraceLength,
				config.testing_maxTraceLength);
		...
		while (!done) {
			...
			logger.info("alphabet:"
					+ SutInfo.alphabetToString(flattenedInputAlphabet));
			...
			try {
				...
				while (!done) {
					...
					processed = learner.addCounterExample(
							o.getCounterExample(), o.getOracleOutput());
		...
		if (config.learnResults_writeAbstractModelPdfFile) {
			DotUtil.invokeDot(new File(config.learnResults_outputDir
					+ config.learnResults_abstractModelDotFile), "pdf",
					new File(config.learnResults_outputDir
							+ config.learnResults_abstractModelPdfFile));
		}
	...
	public static void main(String[] args) {
		...
		try {
			try {
				...
				config = new Config(args); // simply loads params from config
											// file
				...
				System.out.flush();
				System.err.flush();
				util.Time.millisleep(1000); // makes sure exception stacktrace
											// is printed after logger messages
				if (!Config.isLog4jConfigured()) {
					// exception occurred before logging initialized: switch to
					// basic log4j
					BasicConfigurator.configure();
					// get the containing repository
					LoggerRepository repository = logger.getLoggerRepository();
					// Set the hierarchy-wide threshold to WARN effectively
					// disabling
