package sutInterface.tcp.functionalMappers;
...
public class TCPMapperSpecification {
	public static final Long NOT_SET = -3L; //Integer.MIN_VALUE;
	...
	private boolean isLastResponseTimeout, waitingForSyn;
	...
	public TCPMapperSpecification clone() {
		...
		mapper.waitingForSyn = this.waitingForSyn;
		return mapper;
	}
...
//		}
		...
		if (this.freshSeqEnabled || (this.waitingForSyn && flags.is(Flag.SYN))) {
		...
		if (this.freshAckEnabled || (this.waitingForSyn && flags.is(Flag.SYN))) {
		...
		if (this.waitingForSyn && flags.has(Flag.SYN)) {
			this.waitingForSyn = false;
		}
	...
	public String processIncomingResponse(FlagSet flags, long concreteSeq,
		...
		if (!this.waitingForSyn && flags.is(Flag.SYN)) {
			this.waitingForSyn = true;
		}
	...
	private Symbol getAbstract(Long nrReceived, boolean isIncomingSeq) {
		...
		/*if (!this.freshAckEnabled & this.freshSeqEnabled & flagsReceived.is(Flag.SYN)) {
			checkedSymbol = Symbol.SIMULTANEOUS;
		} else */if (learnerSeq != null && nrReceived == Calculator.next(this.learnerSeq)) {
			checkedSymbol = Symbol.SNCLIENTP1;
		} else if (learnerSeq != null && nrReceived == Calculator.nth(this.learnerSeq, 2)) {
			checkedSymbol = Symbol.SNCLIENTP2;
		} else if (learnerSeq != null && nrReceived == this.learnerSeq) {
			checkedSymbol = Symbol.SNCLIENT;
		} else if (sutSeq != null && nrReceived == this.sutSeq) {
			checkedSymbol = Symbol.SNSERVER;
		} else if (sutSeq != null && nrReceived == Calculator.next(this.sutSeq)) {
			checkedSymbol = Symbol.SNSERVERP1;
		} else if (sutSeq != null && nrReceived == Calculator.nth(this.sutSeq, 2)) {
			checkedSymbol = Symbol.SNSERVERP2;
		} else if (lastConcSeqSent != null && nrReceived == this.lastConcSeqSent) {
			checkedSymbol = Symbol.SNSENT;
		} else if (lastConcAckSent != null && nrReceived == this.lastConcAckSent) {
